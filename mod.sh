...
type RefreshCookiesResponse struct {
    Message   string `json:"message,omitempty"`
    Error     string `json:"error,omitempty"`
...
func main() {
    ...
    http.HandleFunc("/api/search", func(w http.ResponseWriter, r *http.Request) {
        ...
        // Try to load cookies from session if available
        session, _ := getSession(r)
        if session != nil {
            loadCookiesToAPI(&resyAPI, session)
        }
    ...
    // Refresh Imperva cookies endpoint
    http.HandleFunc("/api/refresh-cookies", func(w http.ResponseWriter, r *http.Request) {
        if r.Method != http.MethodPost {
            http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
            return
        }

        var refreshReq RefreshCookiesRequest
        if err := json.NewDecoder(r.Body).Decode(&refreshReq); err != nil {
            sendJSONResponse(w, RefreshCookiesResponse{Error: "Invalid request format"}, http.StatusBadRequest)
            return
        }

        if refreshReq.VenueID == 0 {
            // Try to get venue ID from session
            venueIDStr, err := getCookieValue(r, "venue_id")
            if err != nil {
                sendJSONResponse(w, RefreshCookiesResponse{Error: "Venue ID not found. Please provide venue_id or select a restaurant first."}, http.StatusBadRequest)
                return
            }
            refreshReq.VenueID, err = strconv.ParseInt(venueIDStr, 10, 64)
            if err != nil {
                sendJSONResponse(w, RefreshCookiesResponse{Error: "Invalid Venue ID"}, http.StatusBadRequest)
                return
            }
        }

        appendLog("Fetching Imperva cookies for venue: " + strconv.FormatInt(refreshReq.VenueID, 10))
        cookieData, err := imperva.FetchCookiesForAPI(refreshReq.VenueID)
        if err != nil {
            appendLog("Failed to fetch Imperva cookies: " + err.Error())
            sendJSONResponse(w, RefreshCookiesResponse{Error: "Failed to fetch cookies: " + err.Error()}, http.StatusInternalServerError)
            return
        }
        ...
        // Serialize cookies to JSON and store in session
        cookiesJSON, err := json.Marshal(cookieData.Cookies)
        if err != nil {
            appendLog("Failed to serialize cookies: " + err.Error())
            sendJSONResponse(w, RefreshCookiesResponse{Error: "Failed to serialize cookies"}, http.StatusInternalServerError)
            return
        }

        session["imperva_cookies"] = string(cookiesJSON)
        session["imperva_user_agent"] = cookieData.UserAgent

        encoded, err := s.Encode("session", session)
        if err != nil {
            appendLog("Failed to encode session: " + err.Error())
            sendJSONResponse(w, RefreshCookiesResponse{Error: "Failed to save cookies"}, http.StatusInternalServerError)
            return
        }
        ...
        appendLog("Successfully fetched and stored " + strconv.Itoa(len(cookieData.Cookies)) + " Imperva cookies")
        sendJSONResponse(w, RefreshCookiesResponse{
            Message:     "Cookies refreshed successfully",
            CookieCount: len(cookieData.Cookies),
        }, http.StatusOK)
    })
    ...
    http.HandleFunc("/api/reserve", func(w http.ResponseWriter, r *http.Request) {
        ...
        // Load Imperva cookies from session and set on API client
        loadCookiesToAPI(&resyAPI, session)
    ...
    // Start server or CLI based on arguments
...
func handleScheduledReservations(appCtx app.AppCtx) {
    for {
        ...
        if earliestTime.After(now) {
            ...
            // Note: Scheduled reservations run in background without HTTP context
            // For best results, refresh Imperva cookies before scheduling a reservation
            // The cookies should be loaded via the /api/refresh-cookies endpoint first
...
// loadCookiesToAPI loads Imperva cookies from session and sets them on the API client
func loadCookiesToAPI(api *resy.API, session map[string]string) {
    cookiesJSON, ok := session["imperva_cookies"]
    if !ok || cookiesJSON == "" {
        // No cookies in session, API will use default behavior
        return
    }
    ...
    if err != nil {
        appendLog("Failed to deserialize cookies from session: " + err.Error())
        return
    }

    userAgent := session["imperva_user_agent"]
    api.SetCookies(cookies, userAgent)
    appendLog("Loaded " + strconv.Itoa(len(cookies)) + " Imperva cookies into API client")
}
